...
export interface HfHubShape {
  readonly status: () => Effect.Effect<{ authenticated: boolean; user: string | null }>
  ...
  readonly checkpointSteps: (repoId: string) => Effect.Effect<ReadonlyArray<string>>
}
    ...
    Effect.gen(function* () {
      ...
      const getJson = (url: string) =>
        Effect.gen(function* () {
          ...
          if (token) req = HttpClientRequest.setHeader(req, 'authorization', `Bearer ${token}`)
          const res = yield* client.execute(req)
          if (res.status >= 400) return yield* Effect.fail(new Error(`HF ${res.status} for ${url}`))
