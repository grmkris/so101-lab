...
export const Route = createFileRoute('/trainings/$runId')({ component: RunPage })
...
function RunPage() {
  ...
  const saveFinding = useMutation({
    mutationFn: (value: string) => patchRun(runId, { status: null, hypothesis: null, finding: value }),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['runs'] }),
  })

  const markLaunched = useMutation({
    mutationFn: () => patchRun(runId, { status: 'launched', hypothesis: null, finding: null }),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['runs'] }),
  })

  if (run.isPending) return <p className="p-6 text-muted-foreground">loading…</p>
  if (run.isError) return <p className="p-6 text-red-500">failed: {String(run.error)}</p>
  const r = run.data

  const targetSteps = r.config?.steps ?? null
  const ckptSteps = checkpoints.data?.steps ?? []
  const lastCkpt = ckptSteps.at(-1) ? Number.parseInt(ckptSteps.at(-1) as string, 10) : 0
  const progress = targetSteps ? Math.min(100, Math.round((lastCkpt / targetSteps) * 100)) : null
    ...
    <div className="max-w-3xl p-6">
      ...
      {r.config && (
        <div className="mt-4 rounded border p-4 text-sm">
          ...
          <div className="mt-1 font-mono text-muted-foreground">
            ...
            {r.config.episodes ? ` · episodes ${r.config.episodes}` : ' · all episodes'}
            {r.config.pretrainedPath
              ? ` · warm-start ${r.config.pretrainedPath}`
              : ' · from scratch'}
          </div>
          <div className="mt-1 text-muted-foreground">
            {r.config.steps} steps · batch {r.config.batchSize} · save every {r.config.saveFreq}
          </div>
        </div>
      )}
      ...
      <div className="mt-4 rounded border p-4 text-sm">
        ...
        {checkpoints.isPending ? (
          ...
          <div className="mt-2">
            <div className="font-mono text-muted-foreground">{ckptSteps.join(' · ')}</div>
            {progress !== null && (
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex gap-2">
              <button
                ...
                onClick={() => navigator.clipboard.writeText(r.colabCell as string)}
              >
                copy
              </button>
              {r.status === 'draft' && (
