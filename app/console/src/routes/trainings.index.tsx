...
export const Route = createFileRoute('/trainings/')({ component: TrainingsPage })
            ...
            {runs.data.map((r) => (
              <tr key={r.id} className="border-b last:border-0">
                <td className="py-2 pr-4 font-mono">
                  <Link className="underline" to="/trainings/$runId" params={{ runId: r.id }}>
                    {r.name}
                  </Link>
                </td>
                <td className={`py-2 pr-4 ${statusColor[r.status] ?? ''}`}>{r.status}</td>
                <td className="py-2 pr-4 font-mono">{r.config?.datasetRepoId ?? '—'}</td>
                <td className="py-2 pr-4">{r.config?.steps ?? '—'}</td>
                <td className="py-2 pr-4">{r.createdAt?.slice(0, 10) ?? '—'}</td>
                <td className="py-2 max-w-md truncate">{r.hypothesis ?? '—'}</td>
              </tr>
            ))}
          </tbody>
        </table>
