...
- **Micro-chess engineering package (2026-08-18):** `chess_system/` + `docs/chess-system/`. Fixed 8×8 design is 23 mm squares / 184 mm playfield / 204 mm carrier, square centers 97–270 mm from pan pivot. Shared manifest drives fabrication, Blender, MuJoCo, Isaac, teleop and validation. All 64 downward grasps pass at ≤3 mm / ≥5° margin with the 20 mm tool + **24 mm** pieces (squat redesign 2026-08-19: a 32 mm piece is geometrically caged on a crowded back rank — it cannot clear its neighbors before the arm hits its vertical-lift limit). Deterministic RRT-Connect motion planning lands 130 baseline routes (64 exits + 64 entries + 2 capture bins), all passing 16-seed tolerance replay; 50/50 physics replays pass; 27 contract/rule/safety tests pass. **Legality ≠ reachability** — some legal moves (e.g. bishop f1→e2 with f2 occupied) are mechanically inaccessible; the planner must treat executability as a move-selection constraint. BLOCKING before full print: physical five-piece crowded coupon (29/30, zero neighbor contact). Generated finger-extension roots are fit-check prototypes until caliper-measured.
