...
6. **LeRobot's follower writes P=16 on every connect** ("avoid shakiness"; firmware default 32) — a dead band where a ≤2° commanded step never overcomes friction+gravity: the goal reaches the servo and nothing moves (trace 09-17: wrist_flex 0.00° at P=16 vs 1.41° at P=32). Teleop and policies hide it (moving targets, no per-step settle check); bounded agent steps expose it. Fix: robo-harness profile `p_coefficients` 32 on arm joints, gripper 16. Diagnose with `robo-harness/scripts/servo_step_trace.py`, not by blaming the model. **P=32/48 is not enough in the loaded direction** (09-17 night): every joint settles ~0.7° short of a 1.6° step, and `shoulder_lift` raising the extended arm does not move at all — it stalled 30 of 34 climb moves, which left a search sweeping blind 3.5 cm over the mat. **Resolved 09-18 by a P sweep in the loaded pose** (`var/servo-trace-2026-09-18/`): raising 1.8° gave 0.61° at P48, 1.32° at P64, **1.85° at P96** (403 mA) and 1.76° at P128 (566 mA); pan's residual 0.39° at 32 → 0.05° at 64; wrist_flex's return residual 0.79° at 32 → 0.26° at 64. Zero rest jitter at every P, servos ≤41 °C. **Profile now: pan 64, lift 96, elbow 96, wrist_flex 64, roll 32, gripper 16** (the driver's ceiling moved 64 → 128 to allow it). Trace the joint in the pose that actually loads it — the same joint at neutral looked fine at 48.

## The room host — `lab-pi` (2026-08-23)
Pi 4B 4GB, SSD at `/data`, lerobot 0.6.0 + placo, LCD status display. Full build log and
every measured number: `notes/lab-setup-2026-08.md`. **The wrist Innomaker drops off the USB bus under arm motion** and comes back re-enumerated behind the same udev name; the stale handle then reads nothing forever, which looks exactly like a dead camera (it ended a recording and refused every move of a run before `lab_cameras` learned to reopen a device that has delivered nothing for 1 s, 09-18). A sub-second camera hiccup also trips `robo-io`'s 500 ms freshness guard, which stops agent motion and drops the lease — correct, and callers must treat it as transient rather than fatal. `robo-io` encodes camera frames only on demand since 09-17 (eager 30 fps JPEG+base64 of both cameras cost half a core and starved the motor loop into "Control loop deadline missed"). Since 09-06 the arm and cameras are owned by robo-harness `robo-io` (`sudo systemctl stop|start robo-io`; conflicts with `labcam-preview`; not enabled at boot; stopping keeps torque on — cut servo power to go limp; startup refuses a joint left past its calibration range). A follower USB board can stop enumerating after a power pull (`device not accepting address`) — full power cycle of Pi + arms. The four that bite:
...
- **Agent control of the real arm works:** robo-harness (`~/code/robo-harness`, netcup coordinator + lab-pi `robo-io`) runs a decision runner (`bun run jev`) where rules or TypeSafe **Jev** (Vercel AI Gateway, paid credits) pick the next action; control smoke passes with rules / Jev choice / Jev critic after the P=32 fix (journal 09-17).
- **The decision layer is now skill-level** (jev-drone pattern): the model picks skills (scan / centre / descend / grasp / lift), code owns the perception and control loops and keeps the veto. Search sweeps a serpentine raster from a vantage height; centring is a derivative-free hill climb in image space. Passes end-to-end on a simulated arm; recordings work again (2355 samples + MP4 on 09-17).
- **BLOCKED on servo gain, not software** (09-17 night, numbers in `journal.md`): bounded upward/outward moves stall, so the real pickup has not happened. Next session starts with a P sweep under load on `shoulder_lift`/`elbow_flex`, then retry the recorded pickup.
